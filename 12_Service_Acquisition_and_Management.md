...
This page documents the customer journey for acquiring and managing optional vehicle services (opciones disponibles) after vehicle delivery. It covers the complete workflow from service discovery thro[...]  

For the initial vehicle purchase and delivery process, see [Vehicle Purchase and Delivery Process](#4.1). For technical details of customer-facing web and mobile platforms, see [Customer-Facing Platfo[...]  
...
The service acquisition process begins when a customer accesses their expediente de compra through either the web platform or mobile application. Customers can browse the catalog of opciones disponibl[...]  
...
Customers access the service catalog through their expediente de compra, which is accessible via both web and mobile channels. The catalog displays only the opciones disponibles that are compatible wi[...]  
...
graph TB
    ...
    FILTER_ENGINE -.->|Filters out incompatible services| CATALOG_API
    VSS_CLIENT -.->|Checks maintenance for gated services| CATALOG_API
```
...
Certain services have dependencies on the vehicle's official maintenance status. Before allowing purchase of these services, the system queries the VSS (Vehicle Service System) to verify current maint[...]  
...
Payment for services occurs before OTA activation. The system integrates with the payment gateway to process card payments, with CaaS assuming settlement risk by delivering services immediately upon p[...]  
...
**Sources:** [enunciado.md:18-20](), [pasame las preguntas y sus respuestas a markdown.md:60-73](), [pasame las preguntas and sus respuestas to markdown.md:77-82]()
